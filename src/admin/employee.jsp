...
<div class="wrapper">
  ...
  <div class="content table-responsive table-full-width" style="font-size:12px;">
	...
	<tr>
		...
		<td><img src="employee/getemployeeadminImage.jsp?employee_id=${ee.getEmployee_id()}" width="30px" height="30px"/>&nbsp;&nbsp;<a href="employee/viewemployeeadminImage.jsp?employee_id=${ee.getEmployee_id()}">View Image</a></td>
		<td>${ee.getF_name()}</td>
		<td>${ee.getM_name()}</td>
		<td>${ee.getL_name()}</td>
		<td>${ee.getBdate()}</td>
		...
		<td><a href="employee/employeeadmindelete.jsp?employee_id=${ee.getEmployee_id()}">Delete</a></td>
	</tr>
	</c:forEach>
	</tbody>
			</table>
		 	...
		 	<div class="col-md-2">
                <button class="btn btn-default btn-block" onclick="javascript:window.open('../addemployeeadminform.jsp','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,rezisable=yes,width=200px,heightt=200px'); return false;">Add Employee</button>
            </div>
		</div>
		</div>
		</div>
